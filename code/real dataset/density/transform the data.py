...
# 检测文件编码
with open('Original variables.csv', 'rb') as file:
    result = chardet.detect(file.read())  # 或使用 file.read(100000) 检测部分数据

print(f"Detected encoding: {result['encoding']}")

# 使用检测到的编码读取文件
try:
    df = pd.read_csv('Original variables.csv', encoding=result['encoding'])
    print("Data loaded successfully!")
    print(df.head())
except Exception as e:
    print(f"Failed to load data with detected encoding: {e}")

variables = df.iloc[:,0:7].values.T
print(variables.shape)
labels = df.iloc[:,7]
print(labels.shape)
# print(variables.shape)
# labels = np.load("Original Dataset_labels.npy", allow_pickle=True)[0:3000]
row_variances = np.var(variables, axis=1, ddof=1)
row_std = np.sqrt(row_variances)
std = 0.5 * row_std.mean()
noise_dimensions = 93  # 要添加的噪声维度数量
low_dim = 20
...
np.save('Original Dataset_labels.npy', labels, allow_pickle=True)
