...
# # 生成测量矩阵(Toeplitz方式)
# rho = 0.9         # 相关性系数
# Sigma = np.array([[rho**abs(i-j) for j in range(high_dim)] for i in range(high_dim)])
# A = np.random.multivariate_normal(mean=np.zeros(high_dim), cov=Sigma, size=low_dim)

# rho = 0.9
# block_size = 10   # 每个块10个特征
# num_blocks = high_dim // block_size
# Sigma = np.zeros((high_dim, high_dim))
# for i in range(num_blocks):
#     start = i * block_size
#     end = start + block_size
#     Sigma[start:end, start:end] = rho
# np.fill_diagonal(Sigma, 1)  # 对角线为1
# A = np.random.multivariate_normal(mean=np.zeros(high_dim), cov=Sigma, size=low_dim)
